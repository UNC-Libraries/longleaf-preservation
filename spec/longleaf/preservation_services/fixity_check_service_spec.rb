...
describe Longleaf::FixityCheckService do
  ...
  describe '.initialize' do
    context 'no algorithms configured' do
      ...
      it {
        expect { FixityCheckService.new(service_def, app_manager) }.to raise_error(ArgumentError,
  ...
  describe '.is_applicable?' do
    context 'with valid algorithms' do
      ...
      it "returns true for preserve event" do
        expect(fixity_service.is_applicable?(PRESERVE_EVENT)).to be true
      end
  ...
  describe '.perform' do
    ...
    context 'with default absent_digest behavior' do
      ...
      context 'file with missing checksum' do
        ...
        it {
          expect { fixity_service.perform(file_rec, PRESERVE_EVENT) }.to raise_error(ChecksumMismatchError,
    ...
    context 'with absent_digest behavior set to fail' do
      ...
      context 'file with matching checksum' do
        ...
        it { expect { fixity_service.perform(file_rec, PRESERVE_EVENT) }.to_not raise_error }
      end
      ...
      context 'file with incorrect checksum' do
        ...
        it { expect { fixity_service.perform(file_rec, PRESERVE_EVENT) }.to raise_error(ChecksumMismatchError) }
      end
    ...
    context 'with absent_digest behavior set to generate' do
      ...
      context 'file with missing checksum' do
        ...
        it "metadata updated with missing checksum" do
          ...
          expect { fixity_service.perform(file_rec, PRESERVE_EVENT) }.to_not raise_error
    ...
    context 'with multiple configured checksums' do
      ...
      context 'file contains unconfigured checksum' do
        let(:md_rec) {
          build(:metadata_record, checksums: {
            'sha1' => SHA1_DIGEST,
            'md5' => MD5_DIGEST,
            'indigestion' => 'nope' } )
        }
    ...
    context 'with all checksums configured' do
      let(:service_def) {
        make_service_def(['sha1', 'md5', 'sha2', 'sha384', 'sha512', 'rmd160'],
