...
describe Longleaf::DeregisterEvent do
  ...
  describe '.initialize' do
    context 'without a file record' do
      ...
      it { expect { Longleaf::DeregisterEvent.new(file_rec: nil, app_manager: app_config) }
          .to raise_error(ArgumentError, /Must provide a file_rec parameter/) }
      it { expect { Longleaf::DeregisterEvent.new(file_rec: 'file', app_manager: app_config) }
    ...
    context 'without an application config manager' do
      ...
      it { expect { Longleaf::DeregisterEvent.new(file_rec: file_rec, app_manager: nil) }
          .to raise_error(ArgumentError, /Must provide an ApplicationConfigManager/ ) }
      it { expect { Longleaf::DeregisterEvent.new(file_rec: file_rec, app_manager: 'bad') }
  ...
  describe '.perform' do
    ...
    context 'on a deregistered file' do
      ...
      context 'with force flag' do
        let(:event) { Longleaf::DeregisterEvent.new(file_rec: file_rec,
