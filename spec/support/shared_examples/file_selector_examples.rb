RSpec.shared_examples 'file_selector.initialize' do |class_sym|
  describe '.initialize' do
    context 'no file paths or storage locations' do
      it { expect{ build(class_sym, file_paths: nil, storage_locations: nil, app_config: app_config) }.to \
          raise_error(ArgumentError) }
    end
  
    context 'with empty file paths' do
      it { expect{ build(class_sym, file_paths: [], storage_locations: nil, app_config: app_config) }.to \
          raise_error(ArgumentError) }
    end
  
    context 'both file paths and storage locations' do
      it { expect{ build(class_sym, 
